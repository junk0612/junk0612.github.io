...
1. 今回定義した非負整数の文法に、負の整数を表現するための規則を追加してください。負の整数は、先頭にマイナス記号 `-` がつくことを考慮してください。
2. 今回定義した非負整数の文法に、浮動小数点数を表現するための規則を追加してください。浮動小数点数は、整数部と小数部があり、小数部は `.` の後に1つ以上の数字が続くことを考慮してください。
3. \[発展\] 今回定義した非負整数の文法は、日常生活で使われる非負整数の表現と一部異なる部分があります。それはどこでしょうか。わかったら、そのケースを考慮した文法を追加してください。
